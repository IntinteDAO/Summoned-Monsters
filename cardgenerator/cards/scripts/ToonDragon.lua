function c1165.initial_effect(c)
	...
	e1:SetOperation(c1165.atklimit)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e4:SetCondition(c1165.sdescon)
	e4:SetOperation(c1165.sdesop)
	c:RegisterEffect(e4)
	--direct attack
	local e5=Effect.CreateEffect(c)
	e5:SetType(EFFECT_TYPE_SINGLE)
	e5:SetCode(EFFECT_DIRECT_ATTACK)
	e5:SetCondition(c1165.dircon)
	c:RegisterEffect(e5)
	--attack cost
	local e6=Effect.CreateEffect(c)
	e6:SetType(EFFECT_TYPE_SINGLE)
	e6:SetCode(EFFECT_ATTACK_COST)
	e6:SetCost(c1165.atcost)
	e6:SetOperation(c1165.atop)
	c:RegisterEffect(e6)
	end
...
function c1165.dirfilter1(c)
	return c:IsFaceup() and c:IsCode(1061)
end
function c1165.dirfilter2(c)
	return c:IsFaceup() and c:IsType(TYPE_TOON)
end
function c1165.dircon(e)
	return Duel.IsExistingMatchingCard(c1165.dirfilter1,e:GetHandlerPlayer(),LOCATION_ONFIELD,0,1,nil)
		and not Duel.IsExistingMatchingCard(c1165.dirfilter2,e:GetHandlerPlayer(),0,LOCATION_MZONE,1,nil)
end
function c1165.condition(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp
end
function c1165.operation(e,tp,eg,ep,ev,re,r,rp)
