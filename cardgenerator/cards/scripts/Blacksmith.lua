function c1189.initial_effect(c)
	...
	e1:SetCost(c1189.cost)
	e1:SetTarget(c1189.target)
	e1:SetOperation(c1189.operation)
	c:RegisterEffect(e1)
end
function c1189.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	e2:SetOperation(c1189.tgop)
	e2:SetLabel(cid)
	e2:SetReset(RESET_CHAIN)
	Duel.RegisterEffect(e2,tp)
end
function c1189.tgop(e,tp,eg,ep,ev,re,r,rp)
...
function c1189.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return e:IsHasType(EFFECT_TYPE_ACTIVATE)
		and Duel.IsExistingTarget(c1189.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c1189.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
...
function c1189.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		e4:SetCondition(c1189.atkcon)
		e4:SetValue(c1189.atkval)
