function c1196.initial_effect(c)
	...
	e1:SetOperation(c1196.activate)
	c:RegisterEffect(e1)
end
function c1196.filter(c,tp)
	return Duel.GetMZoneCount(tp,c,tp,LOCATION_REASON_CONTROL)>0
end
function c1196.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	if chk==0 then return Duel.IsExistingTarget(c1196.filter,tp,0,LOCATION_MZONE,1,nil,1-tp)
		and Duel.IsExistingTarget(Card.IsAbleToChangeControler,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g1=Duel.SelectTarget(tp,c1196.filter,tp,0,LOCATION_MZONE,1,1,nil,1-tp)
...
function c1196.activate(e,tp,eg,ep,ev,re,r,rp)
